...
import FloodyBot from "@/app/content/code/floodybot/page";
import AbletonFB from "@/app/content/code/abletonfb/page";

interface cardProps {
  title: string;
}
const ProjectCard: React.FC<cardProps> = ({title}) => {
    const post1 = <AbletonFB/>;
    const post2 = <FloodyBot/>;
    ...
    return(
        ...
        <div className="w-[80vh] h-[50vh] backdrop-blur-md shadow-xl rounded-lg bg-radial-[at_0%_15%] from-white/50 to-zinc-900/30 to-100% justify-center items-center flex flex-col  ">
            ...
            <div className="w-98/100 h-98/100 text-gray-800 overflow-auto rounded-b-lg">
                <CardItem text="Ableton File Browser" content={post1}/>
                <CardItem text="3D Flood Prediction: FloodyBot" content={post2}/>
            </div>
        </div>
        </div>
    );
